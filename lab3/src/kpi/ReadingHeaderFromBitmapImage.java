...
public class ReadingHeaderFromBitmapImage {
    ...
    // метод, який приймає потік даних зчитаних із файлу із зображенням та повертатиме об’єкт типу HeaderBitmapImage, з інформацією про зображення
        ...
        while ((line = reader1.read())!=-1) {
            ...
            if (i == 18)
            {
                ...
                if((half % 4) != 0) // якщо не ділиться на 4
                    half= (half/4)*4 + 4; // доповнюємо значення половини ширинизображення, щоб вона була кратна 4
            }
    ...
    // метод для коректного читання полів розміром 2 байти у форматі запису little-endian та переводу в десяткову систему числення
    ...
    // метод для коректного читання полів розміром 4 байти у форматі запису little-endian та переводу в десяткову систему числення
